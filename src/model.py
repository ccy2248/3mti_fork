...
def make_1step_sched():
    noise_scheduler_1step = DDPMScheduler.from_pretrained("/data/home/scxk346/run/workspace/3mti/models/sd-turbo", subfolder="scheduler")
...
def my_vae_encoder_fwd(self, sample):          # modified, no Ref_RGB entering skip connection
    sample = self.conv_in(sample)
    l_blocks = []
    # 下采样过程中，仅保存第0个视角（待修复视角）的中间特征
    # sample形状为 (b*v, c, h, w)，其中v=2（0:待修复，1:参考）
    batch_size = sample.shape[0]  # 总批量大小 = b*v
    num_views = 2  # 固定为2个视角（待修复+参考）
    b = batch_size // num_views  # 原始批量大小
    
    for down_block in self.down_blocks:
        # 提取当前特征中第0个视角的部分（shape: (b, c, h, w)）
        # 例如：总批量为2时，取索引0（忽略索引1的参考视角）
        repair_view_feature = sample[::num_views]  # 步长为num_views，只取第0, 2, 4...个样本（均为待修复视角）
        l_blocks.append(repair_view_feature)  # 仅保存待修复视角的中间特征
        sample = down_block(sample)  # 所有视角仍参与下采样（保证U-Net输入完整）
    ...
    self.current_down_blocks = l_blocks  # 仅包含待修复视角的中间特征
    return sample
...
def my_vae_decoder_fwd(self, sample, latent_embeds=None):       # modified, no Ref_RGB entering skip connection
    sample = self.conv_in(sample)
    upscale_dtype = next(iter(self.up_blocks.parameters())).dtype
    # middle
    sample = self.mid_block(sample, latent_embeds)
    ...
    if not self.ignore_skip:
        skip_convs = [self.skip_conv_1, self.skip_conv_2, self.skip_conv_3, self.skip_conv_4]
        num_views = 2  # 固定为2个视角
        batch_size_total = sample.shape[0]  # 总批量 = b*v
        b = batch_size_total // num_views  # 原始批量大小
        # up
        for idx, up_block in enumerate(self.up_blocks):
            # 1. 获取待修复视角的skip特征（已在编码器中筛选）
            repair_skip = skip_convs[idx](self.incoming_skip_acts[::-1][idx] * self.gamma)  # shape: (b, c, h, w)
            # 2. 为参考视角创建零特征（不参与skip connection）
            ref_skip = torch.zeros_like(repair_skip)  # shape: (b, c, h, w)
            # 3. 交替拼接待修复视角的skip特征和参考视角的零特征，恢复总批量维度
            # 例如：[repair_skip[0], ref_skip[0], repair_skip[1], ref_skip[1], ...]
            skip_in = torch.stack([repair_skip, ref_skip], dim=1).flatten(0, 1)  # shape: (b*v, c, h, w)
            
            # 仅待修复视角有实际的skip特征，参考视角为零（不参与融合）
            sample = sample + skip_in
            sample = up_block(sample, latent_embeds)
    else:
        for idx, up_block in enumerate(self.up_blocks):
...
class Difix(torch.nn.Module):
    def __init__(self, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_vae=4, mv_unet=False, timestep=999, no_skip=False):
        ...
        vae = AutoencoderKL.from_pretrained("/data/home/scxk346/run/workspace/3mti/models/sd-turbo/", subfolder="vae")
        ...
        unet = UNet2DConditionModel.from_pretrained("/data/home/scxk346/run/workspace/3mti/models/sd-turbo/", subfolder="unet")
