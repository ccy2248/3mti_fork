...
def main(args):
    accelerator = Accelerator(
        ...
        project_dir=args.output_dir,     # ← 进行修改加上这行
    )
    ...
    while 1:
        ...
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*l_acc):
                # prompt check
                if step < 2:
                    fnames = batch.get("filename", None)
                    prompts = batch.get("caption", None)  
                    print("\n[DEBUG] Training batch prompts:")
                    for f, p in zip(fnames, prompts):
                        print(f"  {f} -> {p}")
                ...
                # x_tgt_pred = net_difix(x_src, prompt_tokens=prompt_tokens)     
